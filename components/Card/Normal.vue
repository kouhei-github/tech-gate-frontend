...
const userAction = reactive<UserAction>({
  good: [] as number[],
  comment: []as number[],
  mark: []as number[]
})
...
const include = (isFlag: boolean, postId: number, action: "good" | "mark") => {
  switch (action){
    case "good":
      return isFlag || userAction.good.includes( postId );
    case "mark":
      return isFlag || userAction.mark.includes( postId );
  }
}
  ...
  <div class="flex items-start h-[180px] w-full">
    <div @click="moveToOtherSite(recommendImage.url)" class="cursor-pointer hover:bg-[#F2F2F2] w-[450px] mx-auto p-[15px]">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center justify-center space-x-2 z-20" @click.stop="mark(props.recommendImage.id)">
          <div class="flex items-center justify-center"  @click.stop="mark(props.recommendImage.id)">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 hover:bg-white" :class="{'text-[#3595E8] fill-[#3595E8]': include(recommendImage.now_bookmarked, recommendImage.id, 'mark'), 'text-[#B6BBC0]' : !include(recommendImage.now_bookmarked, recommendImage.id, 'mark')}" width="18" height="18" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M9 4h6a2 2 0 0 1 2 2v14l-5 -3l-5 3v-14a2 2 0 0 1 2 -2"></path></svg>
          </div>
          <div class="flex items-center justify-center" @click.stop="comment(props.recommendImage.id)">
          ...
          <div class="flex items-center justify-center" @click.stop="good(props.recommendImage.id)">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 hover:bg-white" :class="{'text-[#3595E8] text-lg': include(recommendImage.now_liked, recommendImage.id, 'good'), 'text-[#B6BBC0]' : !include(recommendImage.now_liked, recommendImage.id, 'good')}" width="18" height="18" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M7 11v8a1 1 0 0 1 -1 1h-2a1 1 0 0 1 -1 -1v-7a1 1 0 0 1 1 -1h3a4 4 0 0 0 4 -4v-1a2 2 0 0 1 4 0v5h3a2 2 0 0 1 2 2l-1 5a2 3 0 0 1 -2 2h-7a3 3 0 0 1 -3 -3"></path></svg>
            <span class="ml-1" :class="{'text-[#3595E8]': include(recommendImage.now_liked, recommendImage.id, 'good'), 'text-[#B6BBC0]' : !include(recommendImage.now_liked, recommendImage.id, 'good')}">{{recommendImage.good_num}}</span>
          </div>
